...
function getMdComponents(flowPlaceholders: Map<string, number>): Components {
  let flowIndex = 0
...
export default function ArticlePageClient({ meta, content, locale = 'en' }: ArticlePageClientProps) {
  ...
  const flowPlaceholders = new Map<string, number>()
  const mdComponents = getMdComponents(flowPlaceholders)
  ...
  return (
    <div className="bg-surface min-h-screen print:bg-white">
      ...
      <div className="hidden print:block px-8 py-6 border-b">
        <h1 className="text-2xl font-serif font-bold">{meta.title}</h1>
        ...
        <div className="flex gap-12">
          ...
          <article className="flex-1 min-w-0">
            <div className="max-w-prose">
              ...
              {/* Flow diagrams */}
              {parsed.flowDiagrams.map((diagram, i) => (
                <FlowDiagram key={i} diagram={diagram} />
              ))}
              ...
              {/* FAQ accordion */}
              <FaqAccordion items={parsed.faqItems} />
            </div>
